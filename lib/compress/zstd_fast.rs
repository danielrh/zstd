...
#[no_mangle]
pub unsafe extern "C" fn ZSTD_fillHashTable(mut ms: *mut ZSTD_matchState_t_0,
                                            mut cParams:
    ...
    while ip.offset(fastHashFillStep as isize).offset(-1isize) <= iend {
        ...
        while i < fastHashFillStep {
            ...
            if i == 0i32 as libc::c_uint ||
                   *hashTable.offset(hash as isize) == 0i32 as libc::c_uint {
                *hashTable.offset(hash as isize) = current.wrapping_add(i)
            }
...
pub unsafe extern "C" fn ZSTD_compressBlock_fast(mut ms:
 ...
 -> size_t {
    ...
    let mls: U32 = (*cParams).searchLength;
    let stepSize: U32 = (*cParams).targetLength;
    match mls {
        5 => {
            return ZSTD_compressBlock_fast_generic(ms, seqStore, rep, src,
                                                   srcSize, hlog, stepSize,
...
unsafe extern "C" fn ZSTD_compressBlock_fast_generic(mut ms:
 ...
 -> size_t {
    ...
    let dictIndexDelta: U32 =
        if dictMode as libc::c_uint ==
               ZSTD_dictMatchState as libc::c_int as libc::c_uint {
            prefixStartIndex.wrapping_sub(dictBase.offset_to(dictEnd).expect("bad offset_to")
                                              as libc::c_long as U32)
        } else { 0i32 as libc::c_uint };
    let dictAndPrefixLength: U32 =
        dictStart.offset_to(dictEnd.offset(prefixStart.offset_to(ip).expect("bad offset_to")
                                               as libc::c_long as
                                               isize)).expect("bad offset_to")
            as libc::c_long as U32;
    stepSize =
        (stepSize as
             libc::c_uint).wrapping_add((0 == stepSize) as libc::c_int as
    ...
    if dictMode as libc::c_uint == ZSTD_noDict as libc::c_int as libc::c_uint
       {
        let maxRep: U32 =
            prefixStart.offset_to(ip).expect("bad offset_to") as libc::c_long
                as U32;
        if offset_2 > maxRep {
    ...
    while ip < ilimit {
        ...
        *hashTable.offset(h as isize) = current;
        if dictMode as libc::c_uint ==
            ...
            if dictMode as libc::c_uint ==
                   ZSTD_dictMatchState as libc::c_int as libc::c_uint {
                let dictMatchIndex: U32 = *dictHashTable.offset(h as isize);
                ...
                if dictMatchIndex <= dictStartIndex ||
                    ...
                    offset_1 = offset;
                    ZSTD_storeSeq(seqStore,
                                  anchor.offset_to(ip).expect("bad offset_to")
                                      as libc::c_long as size_t,
            ...
            let offset_0: U32 =
                match_0.offset_to(ip).expect("bad offset_to") as libc::c_long
                    as U32;
            mLength =
        ...
        *hashTable.offset(ZSTD_hashPtr(ip.offset(-2isize) as
        ...
        while ip <= ilimit &&
            ...
            offset_2 = offset_1;
            offset_1 = tmpOff;
            *hashTable.offset(ZSTD_hashPtr(ip as *const libc::c_void, hlog,
                                           mls) as isize) =
...
unsafe extern "C" fn ZSTD_compressBlock_fast_extDict_generic(mut ms:
 ...
 -> size_t {
    let mut repMatchEnd: *const BYTE = 0 as *const BYTE;
    let mut hashTable: *mut U32 = (*ms).hashTable;
    let base: *const BYTE = (*ms).window.base;
